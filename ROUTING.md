...
- Трафик попадает в туннель **только** одним из способов:
  ...
  2. **qwdtt socks / olcrtc** — через локальный SOCKS5-порт
     (`127.0.0.1:1080` / `127.0.0.1:1081`), который нужно указывать в
     приложении/curl/browser или перехватывать отдельными правилами.
...
## 5. Как проверить, что туннель работает
